...
PASS_LINES = [
    ...
    "No side quests today. Pass.",
    "Rest of the slate is a hard pass. I’m not forcing edges that aren’t there.",
]
...
RECAP_LINES = [
    ...
    "Final + record update later. Receipts stay.",
]
...
CTA_LINES = [
    ...
    "If the number’s worse than what I posted, don’t take it.",
]
...
def pick_one_play(events):
    ...
    for _, event in upcoming:
        ...
        def better(p_new, p_old):
            return p_old is None or p_new > p_old

        h2h = [o for o in offers if o["market"] == "h2h" and in_band(o["price"])]
        if h2h:
            best = None
            for o in h2h:
                if best is None or better(o["price"], best["price"]):
                    best = o
            return {
                "sport": best["sport"],
                "event": best["event"],
        ...
        spreads = [
            o for o in offers
            if o["market"] == "spreads" and in_band(o["price"]) and o["point"] is not None
        ]
        if spreads:
            best = None
            for o in spreads:
        ...
        totals = [
            o for o in offers
            if o["market"] == "totals" and in_band(o["price"]) and o["point"] is not None
        ]
        if totals:
            best = None
            for o in totals:
...
def runs_table_id():
    return AIRTABLE_RUNS_TABLE_ID or AIRTABLE_RUNS_TABLE
...
def build_thread(play: dict) -> list[str]:
    ...
    # make tweet #1 unique to avoid X duplicate-content 403
